...
document.addEventListener("DOMContentLoaded", () => {
  ...
  orderNowBtn.addEventListener("click", () => {
    orderSections.forEach(s => s.classList.remove("hidden"));
    myPlateBtn.classList.remove("hidden");
  });
...
document.getElementById("closeFinalModal").addEventListener("click", () => {
      ...
      patties.forEach(p => {
        p.addEventListener("change", () => {
          ...
          checked.forEach(pp => {
            ...
            bunSelect.innerHTML = `
              <option value="round" data-price="${key ? BURGER_PRICE_MAP[key].round : 0}">Round Bun</option>
              <option value="long" data-price="${key ? BURGER_PRICE_MAP[key].long : 0}">Long Bun</option>
            `;
 ...
 $$(".local-extra-no", card).forEach(radio => {
  radio.addEventListener("change", () => {
    ...
    // Sandwich logic: when filling selected -> show slice selector (default 3)
    if (fillings.length) {
      fillings.forEach(f => {
        f.addEventListener("change", () => {
          const checked = fillings.filter(ff => ff.checked);
          const sliceSelector = $(".slice-selector", card);
          qtyList.innerHTML = "";
          if (checked.length === 0) {
            sliceSelector.classList.add("hidden");
            addBtn.classList.add("hidden");
            sectionExtras?.classList.add("hidden");
            return;
          }
          // show slice selector
          sliceSelector.classList.remove("hidden");
          // build qty rows for each filling x selected slice option
          const sel = $(".slice-select", card);
          const selectedSlice = sel ? sel.value : "3";
          // create one combo row per filling (user can change slice dropdown to affect price)
          checked.forEach(ff => {
            const row = document.createElement("div");
            row.className = "option-row";
            const label = document.createElement("div");
            label.textContent = `${ff.dataset.type} - ${selectedSlice} slices`;
            const qctrl = createQtyControl(0);
            qctrl.addEventListener("qtychange", () => {
              if (anySectionHasQty()) {
                sectionExtras?.classList.remove("hidden");
  ...
  radio.addEventListener("change", () => {
    ...
    updateTotals();
      ...
      // when slice dropdown changes, update labels and prices in qtyList
      const sliceSelect = $(".slice-select", card);
      sliceSelect?.addEventListener("change", () => {
        const sel = sliceSelect.value;
        const rows = $$(".option-row", qtyList);
        rows.forEach((r, idx) => {
          const filling = fillings.filter(f => f.checked)[idx];
          if (!filling) return;
          r.querySelector("div").textContent = `${filling.dataset.type} - ${sel} slices`;
        });
      });
    }
...
$$(".local-extra-yes", card).forEach(radio => {
  radio.addEventListener("change", () => {
    ...
    if (qtyBox && !qtyBox.querySelector(".qty-control")) {
      const q = createQtyControl(1); // start with 1
  ...
  radio.addEventListener("change", () => {
    ...
    addBtn?.addEventListener("click", () => {
      ...
      if (section.dataset.item === "sandwich") {
        const sel = $(".slice-select", card);
        const sliceVal = sel ? sel.value : "3";
        const checkedFillings = fillings.filter(f => f.checked);
        const rows = $$(".option-row", qtyList);
        let idx = 0;
        checkedFillings.forEach(f => {
          const qctrl = rows[idx] ? rows[idx].querySelector(".qty-control") : null;
          const qty = qctrl && qctrl.getQty ? qctrl.getQty() : 0;
          if (qty > 0) {
            // price from map
            const key = f.dataset.key || f.dataset.type.toLowerCase();
            const price = SANDWICH_PRICE_MAP[key] ? SANDWICH_PRICE_MAP[key][sliceVal] : 0;
            newItem.parts.push({ label: `${f.dataset.type} - ${sliceVal} slices`, price, qty });
          }
          idx++;
        });
        // section extras radios -> enable global extras if yes
...
localExtrasYes.forEach(radio => {
  ...
  myPlateBtn.classList.add("hidden");
  platePanel.classList.add("hidden");
